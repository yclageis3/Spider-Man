...
class cncn_Spider:
    ...
    def sheng_(self, url):
        ...
        input_sheng = input('请输入省份（0：退出）：')

        for addr, sheng_url in zip(addr_list[1:-1], url_list[1:-1]):  # [15:16]  河南
            if input_sheng:
                if input_sheng != '0':
                    if input_sheng in addr:
    ...
    def shi_(self, sheng_url):
        ...
        if len(dl_list) == 5:  # 不是直辖市
            ...
            input_shi = input('请输入市级（0：退出）：')

            for addr, shi_url in zip(addr_list[1:], url_list[1:]):
                if input_shi:
                    if input_shi != '0':
                        if input_shi in addr:
                            self.shi_name = addr
    ...
    def start_2(self, url):
        ...
        for item in info_list:
            ...
            if len(html.xpath('//div[@class="change_box"]/dl')) == 5:
                self.file_path = '{}\{}\{}\{}'.format(self.path, self.sheng_name, self.shi_name, name)
            else:
                self.file_path = '{}\{}\{}'.format(self.path, self.sheng_name, name)
    ...
    def download_imgs1(self, img_list1, file_path):
        for num, url in enumerate(img_list1, 1):
            ...
            with open(file_path + '\样{}.jpg'.format(num), 'wb')as f:
    ...
    def download_csv(self, name, imgs1, address, opentime, ticket, content1, content2):
        ...
        with open('{}\{}.csv'.format(self.file_path, self.file_name), 'w+', encoding='utf-8-sig', newline='') as f:
            csvObj = csv.writer(f)
            csvObj.writerow(
